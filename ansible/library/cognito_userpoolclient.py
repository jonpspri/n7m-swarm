...
class CognitoUserPoolClientManager:
    ...
    def find_client_by_name(self, user_pool_id, client_name):
        """Find a user pool client by name"""
        try:
...
def build_client_params(module, for_update=False, existing_client=None):
    ...
    elif for_update and existing_client:
        params["ClientName"] = existing_client.get("ClientName")
    ...
    if module.params.get("default_redirect_uri"):
    ...
    # Auth flows
    if module.params.get("explicit_auth_flows"):
    ...
    # Attributes
    if module.params.get("read_attributes"):
        params["ReadAttributes"] = module.params["read_attributes"]
    if module.params.get("write_attributes"):
    ...
    if module.params.get("prevent_user_existence_errors"):
...
def main():
    argument_spec = dict(
        ...
        callback_urls=dict(required=False, type="list", elements="str", default=[]),
        logout_urls=dict(required=False, type="list", elements="str", default=[]),
        ...
        allowed_oauth_flows_user_pool_client=dict(required=False, type="bool", default=True),
        allowed_oauth_scopes=dict(required=False, type="list", elements="str", default=["openid"]),
        supported_identity_providers=dict(required=False, type="list", elements="str", default=["COGNITO"]),
        explicit_auth_flows=dict(required=False, type="list", elements="str"),
        access_token_validity=dict(required=False, type="int"),
        id_token_validity=dict(required=False, type="int"),
        refresh_token_validity=dict(required=False, type="int"),
