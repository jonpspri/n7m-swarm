...
def build_assets(assets_param):
    """Build assets list for API call"""
    if not assets_param:
        return None

    assets = []
    for asset in assets_param:
        asset_entry = {
            "Category": asset["category"],
            "ColorMode": asset["color_mode"],
        }
        if asset.get("bytes"):
            asset_entry["Bytes"] = asset["bytes"]
...
def build_create_params(module):
    ...
    if module.params.get("use_cognito_provided_values"):
        ...
        assets = build_assets(module.params.get("assets"))
        if assets:
...
def needs_update(existing, module):
    ...
    # If use_cognito_provided_values changed
    existing_use_cognito = existing.get("UseCognitoProvidedValues", False)
    desired_use_cognito = module.params.get("use_cognito_provided_values", False)

    if existing_use_cognito != desired_use_cognito:
        return True

    # If not using cognito provided values, check settings and assets
    if not desired_use_cognito:
        if module.params.get("settings") is not None:
        ...
        if module.params.get("assets") is not None:
            # Assets comparison is complex due to structure differences
            # For simplicity, if assets are provided, assume update needed
            return True
...
def main():
    argument_spec = dict(
        ...
        use_cognito_provided_values=dict(required=False, type="bool", default=False),
    ...
    module = AnsibleAWSModule(
        ...
        mutually_exclusive=[
            ["use_cognito_provided_values", "settings"],
            ["use_cognito_provided_values", "assets"],
        ],
    )
    ...
    if branding_id:
        existing_branding = manager.describe_branding(user_pool_id, branding_id, return_merged)
    elif client_id:
        existing_branding = manager.describe_branding_by_client(user_pool_id, client_id, return_merged)
